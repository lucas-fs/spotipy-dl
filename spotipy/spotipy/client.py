...
class Spotify(object):
    ...
    def _internal_call(self, method, url, payload, params):
        ...
        #print("call : "+str(method) +" - "+str(r.url))
    ...
    def playlist_information(self, playlist_id, fields=None):
        ...
        return self._get("playlists/%s" % (playlist_id), fields=fields)

    def playlist_all_tracks(self, playlist_id):
        """ Get playlist information 
            Parameters:
                - playlist_id - the id of the playlist
        """
        ...
        tracks_count = self._get("playlists/%s/tracks" % (playlist_id), offset=offset, limit=limit)['total']
        
        tracks = self._get("playlists/%s/tracks" % (playlist_id), offset=offset, limit=limit)['items']

        if (tracks_count > limit):
            req = 1
            num_req = 0

            if((tracks_count % limit) != 0):
                #print("resto dif 0")
                num_req = round((tracks_count/limit)+0.5)
            else:
                #print("resto == 0")
                num_req = tracks_count/limit
            
            #print("num_req : "+str(num_req))
            offset_calc = 0
            while (req < num_req):
                #print("while req:"+str(req))
                offset_calc += limit
                #print("while offset: "+str(offset_calc))
                tracks.extend(self._get("playlists/%s/tracks" % (playlist_id), limit=limit , offset=offset_calc )['items'])
                req += 1
